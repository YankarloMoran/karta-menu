...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({
  ...
  return (
    <div
      ...
      className="glass-panel-interactive rounded-2xl overflow-hidden cursor-pointer flex flex-col md:flex-row group"
    >
      {/* Dish Image */}
      <div className="relative h-48 md:h-auto md:w-48 flex-shrink-0 overflow-hidden bg-slate-950">
        ...
        />
        {item.is_featured && (
          <span className="absolute top-3 left-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full shadow-lg flex items-center gap-1">
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 mb-3">
            {item.dietary_tags?.map((tag) => (
              <span
                key={tag}
                className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-orange-500/10 text-orange-300 border border-orange-500/20"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
          ...
          <button
            ...
            className="w-full md:w-auto flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold text-xs transition-all shadow-md active:scale-95 group/btn"
          >
