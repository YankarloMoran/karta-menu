...
export default async function RegisterPage({
  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center bg-background px-6 py-12 relative overflow-hidden'>
      ...
      <div className='absolute bottom-0 right-0 w-[400px] h-[400px] bg-accent-emerald/5 rounded-full blur-[100px] -z-10' />
      ...
      <div className='w-full max-w-md animate-slide-up'>
        <div className='text-center mb-10'>
          ...
          <h1 className='text-3xl md:text-4xl font-serif font-bold mb-3 mt-4'>{t('register_title')}</h1>
          <p className='text-foreground/50'>{t('register_subtitle')}</p>
        </div>
        ...
        <div className='glass p-8 rounded-3xl'>
          <form action={register} className='space-y-6'>
            ...
            <div className='space-y-2'>
              <label className='text-sm font-medium ml-1'>{t('restaurant_name')}</label>
              <div className='relative'>
                <Store className='absolute left-4 top-1/2 -translate-y-1/2 text-foreground/30' size={18} />
                <input
                  type='text' name='restaurantName' required placeholder={t('restaurant_placeholder')}
          ...
          <p className='text-center mt-8 text-sm text-foreground/50'>
            {t('register_has_account')} <Link href='/login' className='text-primary font-bold hover:underline'>{t('register_login_link')}</Link>
          </p>
        </div>
      </div>
    </div>
